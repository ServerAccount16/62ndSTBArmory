...
// CBA xeh PREP override
    ...
    #undef PREP
    #define PREP(function) TRIPLES(ADDON,fnc,function) = compile preprocessFileLineNumbers '\MAINPREFIX\PROJECT_PREFIX\SUBPREFIX\DOUBLES(PREFIX,COMPONENT_F)\functions\DOUBLES(fnc,function).sqf'
#else
    #undef PREP
    #define PREP(function) ['\MAINPREFIX\PROJECT_PREFIX\SUBPREFIX\DOUBLES(PREFIX,COMPONENT_F)\functions\DOUBLES(fnc,function).sqf', 'TRIPLES(ADDON,fnc,function)'] call SLX_XEH_COMPILE_NEW
#endif
